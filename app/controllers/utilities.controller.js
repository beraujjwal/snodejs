...
class UtilitiesController extends controller {
  ...
  constructor() {
    super();
    this.Country = this.getModel("Country");
    this.State = this.getModel("State");
  ...
  async get_countries(req, res, next) {
    ...
    this.Country.scope("activeCountries")
      .findAll({
        where: {
          [this.Op.and]: query,
        },
        include: [{ model: this.State, as: "states" }],
      ...
      .then((datas) => {
        if (datas.length) {
          res
            ...
            .json(
              this.Response.success(
                "OK",
                { count: datas.length, datas: datas },
                res.statusCode
              )
            );
        } else {
          res
            ...
            .json(
              this.Response.blank(
                "No country found as per your request!",
                res.statusCode
              )
            );
        }
      ...
      .then((datas) => {
        if (datas.length) {
          ...
          res
            ...
            .json(
              this.Response.blank(
                "No state found as per your request!",
                res.statusCode
              )
            );
        }
        ...
        include: [
          {
            model: this.State,
            as: "state",
            include: [{ model: this.Country, as: "country" }],
          },
        ],
        order: [[order, ordering]],
      })
      .then((datas) => {
        if (datas.length) {
          ...
          res
            ...
            .json(
              this.Response.blank(
                "No city found as per your request!",
                res.statusCode
              )
            );
        }
      ...
      .then((attribute) => {
        res
          ...
          .send({
            ...
            status: true,
            data: attribute,
            message: "Attribute was added successfully!",
          });
      })
      .catch((err) => {
    ...
    this.Country.findOne({
      ...
      .then((data) => {
        if (data) {
          res
            .status(200)
            .json(this.Response.success("OK", { data: data }, res.statusCode));
        } else {
          res
            .status(500)
            .json(this.Response.error("Attributes Not found.", res.statusCode));
        }
      })
      .catch((err) => {
        res
