...
class user extends service {
  ...
  async login(
    ...
    try {
      ...
      //Finding user with set criteria
      let user = await this.getUserDetailsByUsername(username, transaction);

      if (!user) {
        throw new baseError(
            'We are unable to find your account with the given credentials.',
        );
      }
      ...
      const accessToken = await this.generateAccessToken({ userId: user._id, phone: user.phone, email: user.email, tokenSalt})
  ...
  async getUserDetailsByUsername(username, transaction){
    try{

      const criteria = username.match(this.regexEmail) ? { email: username, status: true, isEmailVerified: true, deletedAt: null } : { phone: username, status: true, isPhoneVerified: true, deletedAt: null };
