...
router.group("/v1.0", (versionRouter) => {
  versionRouter.group("/auth", (authRouter) => {
    ...
    authRouter.post(
      "/otp-resend",
      [userValidation.signin],
      exceptionHandler(authController.otpResend)
    );
    authRouter.post(
      "/otp-verify",
      [userValidation.signin],
  ...
  versionRouter.post(
    "/create-carrier-user",
    exceptionHandler(usersController.createNewCarrierUser)
  );

  versionRouter.get(
    "/2fa-barcode",
    [authMiddleware.verifyToken],
    exceptionHandler(usersController.get2FACode)
  );

  versionRouter.post(
    "/verify-2fa",
    [authMiddleware.verifyToken],
    exceptionHandler(usersController.verify2FACode)
  );
});
